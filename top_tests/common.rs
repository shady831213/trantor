use trantor::processor::{ProcessorCfg, Processor};
pub use trantor::global::XLen;
use terminus_spaceport::memory::region::{GHEAP, U64Access, Region, BytesAccess};
use terminus_spaceport::devices::term_exit;
use std::ops::Deref;
use std::path::Path;
use std::cmp::min;
use std::rc::Rc;
use terminus_spaceport::{EXIT_CTRL, space};
...
fn riscv_test(xlen: XLen, name: &str, debug: bool, num_cores: usize) -> bool {
    ...
    let elf = ElfLoader::new(Path::new("top_tests/elf").join(Path::new(name)).to_str().expect(&format!("{} not existed!", name))).unwrap();
    ...
    //add main memory
    register_memory(&bus, "main_memory", 0x80000000, &GHEAP.alloc(0x10000000, 1).expect("main_memory alloc fail!")).unwrap();
...
fn register_region(bus:&Rc<Bus>, name: &str, base: u64, region: &Rc<Region>) -> std::result::Result<(), space::Error> {
    bus.space_mut().add_region(name, &Region::remap(base, &region))?;
    Ok(())
}

fn register_memory(bus:&Rc<Bus>, name: &str, base: u64, mem: &Rc<Region>) -> std::result::Result<(), space::Error> {
            ...
            if let space::Error::Overlap(n, msg) = e {
                if n == "htif".to_string() {
                    ...
                    range0.iter().for_each(|info| {
                        bus.space_mut().add_region(name, &Region::remap_partial(info.base, mem, 0, info.size)).unwrap();
                    });
                    range1.iter().for_each(|info| {
                        bus.space_mut().add_region(&format!("{}_1", name), &Region::remap_partial(info.base, mem, info.base - base, info.size)).unwrap();
                    });
                    Ok(())
                } else {
